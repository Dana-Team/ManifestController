...
func (r *FSRepository) LoadManifest(ctx context.Context, packageName string, id string) (map[string]string, error) {
	...
	for _, p := range filesPath {
		if p.IsDir() {
			res, err := getDirFiles(filepath.Join(dirPath, p.Name()))
			if err != nil {
				return nil, fmt.Errorf("error reading directory %s: %v", filepath.Join(dirPath, p.Name()), err)
			}
...
func getDirFiles (dirPath string) (map[string]string, error){
	log := log.Log
	path := filepath.Join(dirPath)
	filesPath, err := os.ReadDir(path)
	if err != nil {
		return nil, fmt.Errorf("error reading directory %s: %v", dirPath, err)
	}
	result := make(map[string]string)
	for _, p := range filesPath {
		...
		filePath := filepath.Join(path, p.Name())
		b, err := os.ReadFile(filePath)
		if err != nil {
			return nil, fmt.Errorf("error reading file %s: %v", filePath, err)
		}
